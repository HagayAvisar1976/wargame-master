(function hagayBot(){
  ...
  var botName = "Hagay_bot";
  var engineers =["Hagay","Avisar"];

  function hagayBotlogic(){
...
//--------------------DEFENCE
  ...
  var acoe_plan = [
    ...
    {pattern:lightweight_spaceship,c:235,r:2,rotate:true,flipHorizontal:true,flipVertical:true, count: 2, padding_c: -25, padding_r: 0},
    {pattern:lightweight_spaceship,c:3,r:2,rotate:true,flipHorizontal:true,flipVertical:true, count: 2, padding_c: 25, padding_r: 0},

    {pattern:acron,c:10,r:70,rotate:true,flipHorizontal:true,flipVertical:true, count: 10, padding_c: 20, padding_r: 0},
    {pattern:acron,c:230,r:70,rotate:true,flipHorizontal:true,flipVertical:true, count: 10, padding_c: -20, padding_r: 0},
    {pattern:lightweight_spaceship,c:2,r:2,rotate:true,flipHorizontal:true,flipVertical:true, count: 2, padding_c: 25, padding_r: 0},
    {pattern:lightweight_spaceship,c:2305,r:2,rotate:true,flipHorizontal:true,flipVertical:true, count: 3, padding_c: -25, padding_r: 0},

    {pattern:acron,c:230,r:40,rotate:true,flipHorizontal:true,flipVertical:true, count: 10, padding_c: -20, padding_r: 0},
    {pattern:acron,c:10,r:40,rotate:true,flipHorizontal:true,flipVertical:true, count: 10, padding_c: 20, padding_r: 0}
  ...
  function botLogic() {
    ...
    var rows = GameSDK.getMatrixDimensions().rows;
    var generation = GameSDK.getCurrentGeneration();
    var currentBudget  = GameSDK.getMyBudget(botName);
    ...
    if (hasEnoughBudgetForShape(currentBudget,currentShape)) {

      // ------------ calculate the relavent c and r....
      var cPos=(currentShape.c+shapeCounterIndex2*currentShape.padding_c) % cols;
      var rPos=(currentShape.r+shapeCounterIndex2*currentShape.padding_r) % rows;
  ...
  function insertShape(shape,col,row){
    ...
    var flipH = 0;//shape.flipHorizontal? -1:1;
    var flipV = 0;//shape.flipVertical? -1:1;

    for (i in shape.pattern){

      if (shape.rotate){
        retArray.push(new cell(row-flipV *shape.pattern[i][0],col+flipH * shape.pattern[i][1]));

      }
      else{
        retArray.push(new cell(row-flipV *shape.pattern[i][1], col+flipH * shape.pattern[i][0]));
      }
    }
  ...
  function hasEnoughBudgetForShape(budget,shape){
    return (budget >= getShapeSize(shape.pattern));
  }
